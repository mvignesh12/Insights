...
case "$1" in
  start)
    if [[ $(ps aux | grep '[d]eployment.awscodepipeline.AwsCodePipeline' | awk '{print $2}') ]]; then
  ...
  stop)
    ...
    if [[ $(ps aux | grep '[d]eployment.awscodepipeline.AwsCodePipeline' | awk '{print $2}') ]]; then
     sudo kill -9 $(ps aux | grep '[d]eployment.awscodepipeline.AwsCodePipeline' | awk '{print $2}')
    else
